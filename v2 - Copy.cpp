...
bdt buildbdt(const std::vector<std::string>& fvalues) {
	/// write the implementation for the function here
	int height = fvalues[0].size();//get the height of the tree needed from first string(assumed all strings have same length)
	bdt t = buildzerobdt(height);//build a tree with zeros in leaf nodes
	bdt fucker = t;
	for (int i = 0; i<fvalues.size(); i++) {
		std::string string = fvalues[i];
		for (int j = 0; j<string.size(); j++) {
			runtree(fucker, string, j);
		} 
		fucker->val = "1";
		fucker = t;
	}
